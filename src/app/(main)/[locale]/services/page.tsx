...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    title: tSeo('services_title'),
    description: tSeo('services_description'),
    alternates: {
      ...
      <section className="bg-zinc-950 pt-48 pb-24 text-white overflow-hidden relative">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-full text-xs font-bold uppercase tracking-widest text-zinc-400 mb-8">
            <Sparkles size={14} className="text-green-light" /> {tNav('services')}
          </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {serviceKeys.map((key) => {
              ...
              return (
                <div key={key} className="group p-10 bg-white rounded-3xl border border-zinc-100 hover:border-green-primary/20 hover:shadow-2xl hover:shadow-green-primary/5 transition-all">
                  <div className="mb-8 w-16 h-16 bg-zinc-50 rounded-2xl flex items-center justify-center text-zinc-900 group-hover:bg-green-dark group-hover:text-white transition-all">
                    <Icon size={32} />
                  </div>
